...
前往 **[GitHub Releases](https://github.com/rokiai/lazy-ant-app/releases/latest)** 下载对应系统的安装包：

| 系统        | 文件                  | 说明                        |
| :---------- | :-------------------- | :-------------------------- |
| **macOS**   | `LazyAnt-*-arm64.dmg` | Apple Silicon（M 系列芯片） |
| **macOS**   | `LazyAnt-*-x64.dmg`   | Intel 芯片                  |
| **Windows** | `LazyAnt-*-setup.exe` | 64 位安装程序               |
| **Linux**   | `LazyAnt-*.AppImage`  | 免安装，直接运行            |
| **Linux**   | `lazyant_*_amd64.deb` | Debian / Ubuntu 系          |
...
1. 按芯片下载对应 DMG（Apple Silicon 选 `arm64`，Intel 选 `x64`）
...
1. 运行 `LazyAnt-*-setup.exe`
2. 若出现 SmartScreen：点「更多信息」→「仍要运行」

### Linux

- **AppImage**：`chmod +x LazyAnt-*.AppImage && ./LazyAnt-*.AppImage`
- **deb**：`sudo dpkg -i lazyant_*_amd64.deb`（缺依赖时再 `sudo apt -f install`）
